...
public class MinecartNetworkEdit
{
    ...
    {
        return $"[MNE]({TokenParser.ParseText(this.TabName)}){TokenParser.ParseText(this.DisplayName)} RemoveNetwork:{this.RemoveNetwork} RemoveDestinations:{string.Join(',', this.RemoveDestinations)} AppendDestinations:{string.Join(',', this.AppendDestinations)}";
    }
